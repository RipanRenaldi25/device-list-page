...
function Table({ tipe, onTipeChange }) {
  ...
  const realData = useSelector((state) => state.device);
  function onNextPage () {
    nextPage();
  ...
  return (
    <div className="p-4 text-center">
      ...
      <div className="flex justify-center items-center mt-7">
        <button className="mr-3 text-2xl" disabled={!canPreviousPage} onClick={() => onPreviousPage()}>
          &lt;
        </button>
        ...
        <button className="ml-3 text-2xl" disabled={!canNextPage} onClick={() => onNextPage()}>
          &gt;
        </button>
      </div>
    </div>
